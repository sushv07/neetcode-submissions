class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        #Optimal Solution - MinHeap - O(n) + O(nlogk) + O(klogk) => O(nlogk) TC
        #O(n+k) SC
        #Ideal to use when data is streaming and when k is small or k < num of unique elements in the input list
